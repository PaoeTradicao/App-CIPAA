...
function renderizarSelectSetorVistoria() {
  ...
  select.innerHTML = estado.setores.map((setor) => `<option value="${setor.id}">${escaparHtml(limparNomeSetor(setor.nome))}</option>`).join('');
  ...
  const blocoAvulsas = avulsas.length ? `
    ...
    ${avulsas.map((obs) => `
      <div style="margin-bottom:10px;">
        <div class="meta-registro" style="margin-bottom:5px;">${escaparHtml(limparNomeSetor(obs.setorNome) || 'Geral')} · ${obs.criadoEm ? formatarDataHora(obs.criadoEm).dataTexto : '—'} · ${escaparHtml(obs.criadoPor || '')}</div>
  ...
  try {
    ...
    function blocoObservacao(obs, indent) {
      ...
      escrever(`Urgência: ${obs.urgencia.toUpperCase()}${riscoTx}  ·  Status: ${textoStatusObs(obs)}`, {
        tamanho: 8.2, estilo: 'bold', cor: corUrgenciaRgb(obs.urgencia), indent, espacoDepois: 0.6
      });
      escrever(obs.texto, { tamanho: 8.8, indent, cor: corInk, espacoDepois: 0.6 });
    ...
    if (avulsas.length) {
      ...
      avulsas.forEach((obs, indice) => {
        ...
        escrever(`${limparNomeSetor(obs.setorNome) || 'Geral'}   ·   ${dataTx}   ·   ${obs.criadoPor || ''}`, { tamanho: 9.3, estilo: 'bold', espacoDepois: 0.8 });
        blocoObservacao(obs, 3);
        if (indice < avulsas.length - 1) {
          doc.setDrawColor(240, 240, 230);
          doc.line(margem, y - 0.5, margem + largura, y - 0.5);
          y += 1.2;
        }
      });
      y += 1;
    ...
    vistorias.forEach((vistoria) => {
      garantirEspaco(22);
      const { dataTexto, horaTexto } = vistoria.criadoEm ? formatarDataHora(vistoria.criadoEm) : { dataTexto: '—', horaTexto: '—' };
      tituloSecao(limparNomeSetor(vistoria.setorNome).toUpperCase(), corVerdeEscuro);
      const cargoTx = vistoria.colaboradorCargo ? ` (${vistoria.colaboradorCargo})` : '';
      escrever(`${dataTexto} às ${horaTexto}   ·   ${vistoria.colaboradorNome}${cargoTx}`, { tamanho: 8.5, cor: corTextoSuave, espacoDepois: 1.5 });

      const itens = vistoria.itens || [];
      if (itens.length) {
        escrever('Checklist', { tamanho: 9, estilo: 'bold', cor: corInk, espacoDepois: 0.8 });
        itens.forEach((item) => {
          garantirEspaco(6);
      ...
      if (obsDaVistoria.length) {
        escrever('Observações', { tamanho: 9, estilo: 'bold', cor: corInk, espacoDepois: 0.8 });
