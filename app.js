...
function atualizarStatusConexao() {
  ...
  if (navigator.onLine) {
    ...
    texto.textContent = 'offline — sincroniza depois';
  }
}
...
function renderizarListaSetores() {
  ...
  lista.innerHTML = estado.setores.map((setor) => `
    <button type="button" class="item-setor" data-abrir-setor="${setor.id}">
      <div>
        <div class="nome-setor">${escaparHtml(setor.nome)}</div>
        <div class="contagem-itens">${(setor.checklist || []).length} item(ns) no checklist</div>
      </div>
    </button>
  `).join('');
...
function abrirModalSetor(setorId) {
  ...
  abrirModal(setor ? 'Editar setor' : 'Novo setor', `
    <div class="campo">
      ...
      <input type="text" id="modal-nome-setor" value="${setor ? escaparHtml(setor.nome) : ''}" placeholder="Ex.: Padaria, Confeitaria, Estoque" />
    </div>
  ...
  $('#modal-salvar-setor').addEventListener('click', async () => {
    const nome = $('#modal-nome-setor').value.trim();
    if (!nome) {
  ...
  if (botaoExcluir) {
    botaoExcluir.addEventListener('click', async () => {
      abrirModal('Excluir setor', `
        <p style="font-size:0.92rem;margin-bottom:18px;">Tem certeza que deseja excluir "${escaparHtml(setor.nome)}"? As vistorias já registradas continuam no relatório.</p>
...
function renderizarSelectSetorVistoria() {
  ...
  select.innerHTML = estado.setores.map((setor) => `<option value="${setor.id}">${escaparHtml(setor.nome)}</option>`).join('');
  ...
  const existeAinda = estado.setores.some((s) => s.id === valorAtual);
  select.value = existeAinda ? valorAtual : estado.setores[0].id;
  carregarChecklistParaVistoria(select.value);
}

$('#select-setor-vistoria').addEventListener('change', (evento) => {
  carregarChecklistParaVistoria(evento.target.value);
});
  ...
  $('#rapido-salvar').addEventListener('click', async () => {
    ...
    try {
      await db.collection('observacoes').add({
        vistoriaId: null,
        setorId,
        setorNome: setor ? setor.nome : 'Geral',
...
function obterDadosFiltradosRelatorio() {
  ...
  let vistorias = estado.vistorias;
  if (filtroSetor) vistorias = vistorias.filter((v) => v.setorId === filtroSetor);

  let avulsas = estado.observacoes.filter((o) => !o.vistoriaId);
  ...
  return { vistorias, avulsas, filtroSetor, filtroStatus };
}
...
function renderizarRelatorio() {
  ...
  const blocoAvulsas = avulsas.length ? `
    <p class="subtitulo-registro" style="margin-top:0;">Registros avulsos</p>
    ${avulsas.map((obs) => `
      <div style="margin-bottom:10px;">
        <div class="meta-registro" style="margin-bottom:5px;">${escaparHtml(obs.setorNome || 'Geral')} · ${obs.criadoEm ? formatarDataHora(obs.criadoEm).dataTexto : '—'} · ${escaparHtml(obs.criadoPor || '')}</div>
        ${renderizarCardObservacao(obs)}
      </div>
    `).join('')}
  ` : '';

  const blocoVistorias = vistorias.map((vistoria) => {
    const obsDaVistoria = estado.observacoes.filter((o) => {
      if (o.vistoriaId !== vistoria.id) return false;
      if (filtroStatus && o.status !== filtroStatus) return false;
      return true;
    });

    if (filtroStatus && obsDaVistoria.length === 0) return '';
    ...
    const pendentes = itens.filter((i) => !i.checado).length;
    ...
    return `
      <div class="registro-vistoria">
        <button type="button" class="cabecalho-registro" data-abrir-registro="${vistoria.id}" style="width:100%;text-align:left;">
          <div>
            <div class="info-principal">${escaparHtml(vistoria.setorNome)}</div>
            <div class="meta-registro">${dataTexto} ${horaTexto} · ${escaparHtml(vistoria.colaboradorNome)}</div>
          </div>
          <span class="etiqueta-status ${pendentes > 0 ? 'aberta' : 'resolvida'}">${pendentes > 0 ? pendentes + ' pendente(s)' : 'completo'}</span>
        </button>
        <div class="corpo-registro oculto" id="corpo-registro-${vistoria.id}">
          <p class="subtitulo-registro">Checklist</p>
          ${itens.map((item) => `
  ...
  lista.innerHTML = (blocoAvulsas ? `<div class="cartao">${blocoAvulsas}</div>` : '') + blocoVistorias;
...
function renderizarCardObservacao(obs) {
  ...
  return `
    <div class="card-observacao-relatorio">
      ...
      ${resolvida
        ? `<div class="resolucao-info">Resolvida por ${escaparHtml(obs.resolvidoPor)} em ${dataResolucao}</div>`
...
async function exportarRelatorioPDF() {
  ...
  try {
    ...
    function tituloSecao(texto, corTexto) {
      ...
      doc.roundedRect(margem, y - 5.2, largura, 8.2, 1.5, 1.5, 'F');
    ...
    function blocoObservacao(obs, indent) {
      ...
      escrever(`Urgência: ${obs.urgencia.toUpperCase()}${riscoTx}  ·  Status: ${textoStatusObs(obs)}`, {
        tamanho: 8.3, estilo: 'bold', cor: corUrgenciaRgb(obs.urgencia), indent, espacoDepois: 1
      });
      escrever(obs.texto, { tamanho: 9, indent, cor: corInk });
      if (obs.status === 'resolvida') {
        const dataRes = obs.dataResolucao ? formatarDataHora(obs.dataResolucao).dataTexto : '';
        escrever(`Resolvida por ${obs.resolvidoPor || ''} em ${dataRes}`, { tamanho: 8.3, cor: corTextoSuave, indent, estilo: 'italic' });
      }
      y += 1.5;
    }
    ...
    escrever(`Gerado em ${agora.toLocaleDateString('pt-BR')} às ${agora.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}`, { tamanho: 8.5, cor: corTextoSuave, espacoDepois: 1 });

    const { vistorias, avulsas, filtroSetor, filtroStatus } = obterDadosFiltradosRelatorio();
    ...
    escrever(`Filtro aplicado: ${nomeSetorFiltro}  ·  ${nomeStatusFiltro}`, { tamanho: 8.5, cor: corTextoSuave });
    y += 2;
    linhaSeparadora();
    ...
    if (avulsas.length) {
      ...
      avulsas.forEach((obs, indice) => {
        garantirEspaco(24);
        const dataTx = obs.criadoEm ? formatarDataHora(obs.criadoEm).dataTexto : '—';
        escrever(`${obs.setorNome || 'Geral'}   ·   ${dataTx}   ·   ${obs.criadoPor || ''}`, { tamanho: 9.8, estilo: 'bold', espacoDepois: 1.5 });
        blocoObservacao(obs, 3);
        if (indice < avulsas.length - 1) {
          doc.setDrawColor(240, 240, 230);
          doc.line(margem, y - 1, margem + largura, y - 1);
          y += 2;
        }
      });
      y += 2;
    ...
    vistorias.forEach((vistoria) => {
      garantirEspaco(28);
      const { dataTexto, horaTexto } = vistoria.criadoEm ? formatarDataHora(vistoria.criadoEm) : { dataTexto: '—', horaTexto: '—' };
      tituloSecao(vistoria.setorNome.toUpperCase(), corVerdeEscuro);
      const cargoTx = vistoria.colaboradorCargo ? ` (${vistoria.colaboradorCargo})` : '';
      escrever(`${dataTexto} às ${horaTexto}   ·   ${vistoria.colaboradorNome}${cargoTx}`, { tamanho: 8.7, cor: corTextoSuave, espacoDepois: 3 });

      const itens = vistoria.itens || [];
      if (itens.length) {
        escrever('Checklist', { tamanho: 9.3, estilo: 'bold', cor: corInk, espacoDepois: 1.5 });
        itens.forEach((item) => {
          garantirEspaco(7);
          ...
          doc.roundedRect(margem + 2, y - 3.1, 3.4, 3.4, 0.7, 0.7, concluido ? 'FD' : 'D');
          if (concluido) {
            doc.setDrawColor(255, 255, 255);
            doc.setLineWidth(0.5);
            doc.line(margem + 2.6, y - 1.5, margem + 3.3, y - 0.7);
            doc.line(margem + 3.3, y - 0.7, margem + 5, y - 2.6);
          ...
          linhasItem.forEach((l, i) => doc.text(l, margem + 8, y + (i * 3.9)));
          y += Math.max(3.9 * linhasItem.length, 5);
        });
        y += 2.5;
      }
      ...
      if (obsDaVistoria.length) {
        escrever('Observações', { tamanho: 9.3, estilo: 'bold', cor: corInk, espacoDepois: 1.5 });
