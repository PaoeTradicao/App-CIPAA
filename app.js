...
async function exportarRelatorioPDF() {
  ...
  try {
    ...
    function desenharResumoExecutivo(resumo) {
      ...
      const todosChips = [...chipsUrgencia, ...chipsRisco];

      const larguraInterna = largura - 8;
      const linhas = layoutChips(todosChips, margem + 4, larguraInterna);
      const alturaTitulo = 6.5;
      const alturaTotal = alturaTitulo + linhas.length * 6.6 + 3;

      garantirEspaco(alturaTotal + 5);
      doc.setFillColor(245, 250, 209);
      doc.roundedRect(margem, y - 3.5, largura, alturaTotal, 2, 2, 'F');

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(9.8);
      doc.setTextColor(corInk[0], corInk[1], corInk[2]);
      doc.text(`Resumo: ${resumo.total} ${resumo.total === 1 ? 'ocorrência no período' : 'ocorrências no período'}`, margem + 4, y + 1.3);

      desenharChips(linhas, y + 1.3 + alturaTitulo);
    ...
    if (gruposVistorias.length) {
      ...
      gruposVistorias.forEach((grupo) => {
        ...
        escrever(`${grupo.nome}: ${grupo.total} ${grupo.total === 1 ? 'vistoria' : 'vistorias'}${grupo.altaCount ? `, ${grupo.altaCount} observações de alta urgência` : ''}`, {
          tamanho: 9.3, estilo: 'bold', cor: corVerdeEscuro, espacoDepois: 2.5
        });
        ...
        grupo.itens.forEach((vistoria) => {
          ...
          if (itens.length) {
            escrever(`Checklist: ${conformes} de ${itens.length} conforme${conformes === 1 ? '' : 's'}`, {
              tamanho: 8.5, estilo: 'bold', cor: naoConformes.length ? corUrgenciaRgb('alta') : [43, 130, 50], indent: 2, espacoDepois: naoConformes.length ? 1 : 0.5
            });
