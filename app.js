...
function renderizarRelatorio() {
  ...
  const blocoAvulsas = avulsas.length ? `
    <p class="eyebrow" style="margin:0 0 10px;">Registros avulsos</p>
    ${avulsas.map((obs) => renderizarCardAvulso(obs)).join('')}
  ` : '';
  ...
  const rotuloVistorias = (vistorias.length && blocoVistorias)
    ? `<p class="eyebrow" style="margin:${avulsas.length ? '20px' : '0'} 0 10px;">Vistorias completas</p>`
    : '';

  lista.innerHTML = blocoAvulsas + rotuloVistorias + blocoVistorias;
  ...
  return `
    <div class="registro-avulso">
      ...
      <div class="corpo-avulso">
        ...
        ${resolvida
          ? `<div class="resolucao-info">Resolvida por ${escaparHtml(obs.resolvidoPor)} em ${obs.dataResolucao ? formatarDataHora(obs.dataResolucao).dataTexto : ''}</div>`
